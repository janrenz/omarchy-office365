...
- **`src/graph.py`, `src/imapmail.py`, `src/ewscal.py`** — Python stdlib only, one JSON object per call
...
- **`src/Model.js`** — `.pragma library`, no Qt types, already runs under plain
  ...
  written twice.
- **The window** (`src/MailWindow.qml` and everything under it) — a `FloatingWindow`,
...
| `Hyprland.toplevels`, `Hyprland.dispatch` | `MailWindow.qml` — find and focus the existing window | Not needed: a single window process raises itself when asked over IPC. GNOME gives no client a way to focus another's window, so do not try |
...
- **`service` kind (`src/Store.qml`)** is how one store is shared across every
  widget on every monitor. On GNOME the extension is that single owner; the
  window can keep its own `Store` or ask the extension.
- Several helpers (`graph.py`, `imapmail.py`, `ewscal.py`) — all of them move
  as they are.
