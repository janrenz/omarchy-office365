...
QtObject {
  ...
  // a digest ("4 new emails") uses, since a digest is about no single message.
  ...
  function observe(scope, items, present, silent) {
    ...
    if (fresh.length > maxAtOnce) {
      ...
      // it: while a morning's mail lands in batches, "4 new emails" becoming
      // "9 new emails" is one thing to read, not two.
  ...
  property Process proc: Process {
    ...
    onExited: function(exitCode) {
      ...
      if (exitCode !== 0 && item)
        console.warn("mail: could not post a notification:", item.summary)
